...
use gate_http::client::{GateClient, GateClientBuilder, error::ClientError};
...
fn get_base_url() -> String {
    ...
    if let Some(window) = window()
        && let Ok(location) = window.location().origin()
    {
        return location;
    }
...
pub fn get_client() -> Result<GateClient, ClientError> {
