...
impl BootstrapTokenParser {
    ...
    pub async fn find_latest_token(&self) -> Result<Option<String>> {
        ...
        for log_file in log_files.iter().take(5) { // Only check 5 most recent files
...
mod tests {
    ...
    async fn test_find_latest_token_multiple_files() {
        ...
        file.write_all(b"2025-08-14T10:00:00Z  INFO Generated bootstrap token: OlderToken123456789\n").await.unwrap();
        drop(file);
        ...
        file.write_all(b"2025-08-15T15:21:07Z  INFO Generated bootstrap token: NewerToken123456789\n").await.unwrap();
        drop(file);
    ...
    async fn test_find_all_tokens_with_context() {
        ...
        let mut file = tokio::fs::File::create(&log_file_path).await.unwrap();
        file.write_all(b"2025-08-15T10:00:00Z  INFO Generated bootstrap token: Token1234567890ABC\n").await.unwrap();
        file.write_all(b"2025-08-15T11:00:00Z  INFO Some other message\n").await.unwrap();
        file.write_all(b"2025-08-15T12:00:00Z  INFO Generated bootstrap token: Token2345678901DEF\n").await.unwrap();
        drop(file);
