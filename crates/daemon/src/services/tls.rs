...
impl TlsManager {
    ...
    pub async fn set_tls_forward_client(&self, endpoint: Arc<gate_p2p::Endpoint>, node_id: gate_p2p::NodeId) {
                ...
                match cert_mgr.request_certificate(domain, email).await {
                    Ok(()) => {
                        ...
                        if let Ok(new_acceptor) = cert_mgr.get_or_create_tls_acceptor(&domains).await {
                            ...
                            info!("Reloaded TLS acceptor with new certificates for domains: {:?}", domains);
                        }
                    }
                    Err(e) => {
        ...
        tokio::spawn(async move {
            while state_rx.changed().await.is_ok() {
                    ...
                    && last_domain.as_ref() != Some(&assigned_domain)
                {
                    ...
                    info!("TLS forward domain {} detected. Manual config update required for Let's Encrypt.", assigned_domain);
                    
                    // Check if we have a certificate for this domain
                    let cert_mgr = cert_manager.lock().await;
                    if cert_mgr.has_certificate(&assigned_domain).await {
                        info!("Found existing certificate for TLS forward domain: {}", assigned_domain);
                        ...
                        if let Ok(new_acceptor) = cert_mgr.get_or_create_tls_acceptor(&domains).await {
                            ...
                            info!("Reloaded TLS acceptor with TLS forward domain: {}", assigned_domain);
                            last_domain = Some(assigned_domain);
                        }
                    } else {
                        info!("No certificate found for TLS forward domain: {}, will be requested later", assigned_domain);
                        last_domain = Some(assigned_domain);
                    }
                }
            }
