...
use crate::{
    runtime::{inner::RuntimeInner, Runtime},
    Settings, StateDir,
};
...
impl RuntimeBuilder {
    ...
    pub async fn build(self) -> Result<Runtime> {
        info!("Building runtime in {} mode", if self.gui_mode { "GUI" } else { "daemon" });
        ...
        let database_url = self.database_url.unwrap_or_else(|| {
            format!("sqlite://{}", state_dir.data_dir().join("gate.db").display())
        });
        ...
        let static_dir = self.static_dir.or_else(|| {
            if self.gui_mode {
                ...
                std::env::var("GATE_SERVER__STATIC_DIR").ok()
        ...
        let inner = RuntimeInner::initialize(
            settings,
            state_dir,
            database_url,
            static_dir,
